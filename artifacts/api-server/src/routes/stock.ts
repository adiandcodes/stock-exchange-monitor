...
const SCRIPT_PATH = process.env.NODE_ENV === "development" ? path.resolve(_dir, "..", "src", "stock_fetch.py") : path.resolve(_dir, "stock_fetch.py");

function fetchStockData(symbol: string, period: string): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const proc = spawn("python", [SCRIPT_PATH, symbol, period], {
      timeout: 30000,
    });
